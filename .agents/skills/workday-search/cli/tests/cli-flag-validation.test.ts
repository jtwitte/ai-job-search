...
describe("workday CLI flag validation", () => {
  ...
  describe("--page NaN validation", () => {
    test("non-numeric string exits 1 with BAD_ARG", async () => {
      const result = await runCLI(["search", "-s", "example.wd3.myworkdayjobs.com/Careers", "--page", "abc"]);
      expect(result.exitCode).not.toBe(0);
      const err = parsedStderr(result.stderr);
  ...
  describe("--limit NaN validation", () => {
    test("non-numeric string exits 1 with BAD_ARG", async () => {
      const result = await runCLI(["search", "-s", "example.wd3.myworkdayjobs.com/Careers", "--limit", "xyz"]);
      expect(result.exitCode).not.toBe(0);
      const err = parsedStderr(result.stderr);
      expect(err.code).toBe("BAD_ARG");
  ...
  describe("detail requires an id/url", () => {
    ...
    test("bare requisition id without --site exits 1 with BAD_ID", async () => {
      const result = await runCLI(["detail", "R-12345"]);
      expect(result.exitCode).not.toBe(0);
      const err = parsedStderr(result.stderr);
  ...
  describe("no command / help", () => {
    test("no command exits 1 and prints help to stdout", async () => {
      ...
      expect(result.stdout).toMatch(/workday-cli/);
    });
  });
});
