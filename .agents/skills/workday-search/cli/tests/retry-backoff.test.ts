...
describe("textFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const text = await textFetch("https://example.wd3.myworkdayjobs.com/wday/cxs/example/Careers/jobs");
    expect(text).toBe("ok");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(textFetch("https://example.wd3.myworkdayjobs.com/wday/cxs/example/Careers/jobs")).rejects.toThrow(/500/);
