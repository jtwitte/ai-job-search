...
- **Rheinmetall:** `site:rheinmetall.com/de/karriere` OR `site:rheinmetall.recruitmentplatform.com` - WebSearch fallback only. `/add-portal` investigation (2026-08-11) found `rheinmetall.com/de/karriere/aktuelle-stellenangebote` doesn't embed job data at all - search runs entirely through an embedded third-party ATS widget (TalentLink, on `rheinmetall.recruitmentplatform.com` / `emea3.recruitmentplatform.com`) whose `robots.txt` disallows everything for `user-agent: *`, with only Googlebot allowed a narrow static-asset + `/fo/rest` API carve-out. No CLI was built and no unfiltered snapshot was available either (unlike TKMS). Rheinmetall is a declared Target Sector (defence industry, CLAUDE.md).

**Standing reminder (added 2026-08-12):** The WebSearch fallback for both TKMS and Rheinmetall is confirmed unreliable - two real, well-matched postings (`Technischer Manager unbemannte Unterwasserfahrzeuge` at TKMS ATLAS ELEKTRONIK, `Produktmanager Autonome Wasserfahrzeuge (USV)/Drohnenboote` at Rheinmetall/Blohm+Voss) were found only because Jan-Torben spotted them himself browsing the career sites directly - `/scrape` missed both entirely. Since these are two of the strongest-aligned target companies (naval/autonomous-systems work), periodically check `jobs.tkmsgroup.com` and Rheinmetall's careers page by hand rather than trusting `/scrape` to catch everything there. Revisit building a dedicated CLI for either if the robots.txt situation changes or a workable unfiltered snapshot/API surfaces.
...
Jan-Torben is currently based in Jönköping/Huskvarna, Sweden. As of 2026-08-11 he is bound to **Northern Germany, Southern Sweden, and possibly the Copenhagen region** - this is a firmer constraint than "open to relocation," not just a soft preference. When evaluating results, use these tiers:

- **Ideal:** Jönköping / Huskvarna and surrounding area (current home base)
- **Acceptable (bound region):** Northern Germany (Hamburg, Luebeck, Kiel, Rostock, Wismar, Bremen, Wilhelmshaven, Neustrelitz, Elsfleth/Oldenburg area); Southern/Central Sweden (Gothenburg, Halmstad, Helsingborg, Karlshamn, Karlskrona, Lund, Malmoe, Norrkoeping); Copenhagen-Malmoe (Oeresund) area. Norrkoeping was added 2026-08-11 specifically because SMHI (a declared target employer) is based there and Jan-Torben confirmed he'd consider relocating for the right SMHI role, even though it sits north of the otherwise-Southern-Sweden cluster.
- **Outside the bound region:** Everywhere else in Sweden/Germany or further afield. `/rank` filters these out of the default shortlist/below-threshold presentation - **but still surfaces them separately if `rank_score` >= 80** (Strong Fit territory), since an exceptional match is worth knowing about even outside the bound region. Below 80, note the count filtered rather than listing them individually.

This tier list is presentation guidance for `/rank`'s Step 5, not a scoring-dimension change - `04-job-evaluation.md`'s Location dimension still scores PASS/FAIL/FLAG per posting as before; this filter only controls what surfaces by default.
